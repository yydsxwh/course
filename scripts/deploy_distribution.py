...
def pack():
    tmp = os.path.join(tempfile.gettempdir(), "yyds-dist-deploy.tar.gz")
...
def main():
    ...
    c = connect()
    sftp = c.open_sftp()
    remote_tar = "/tmp/yyds-dist-deploy.tar.gz"
    sftp.put(tarball, remote_tar)
    sftp.close()

    # Keep .env and existing db; replace code
    ...
    run(c, "pm2 save")
    run(c, "sleep 2; curl -s -o /dev/null -w '%{http_code}\\n' http://127.0.0.1:3000/studio/distribution")
