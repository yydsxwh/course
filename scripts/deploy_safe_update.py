...
def main() -> int:
    ...
    run(
        ...
        "if integrity != 'ok':\n"
        "    raise SystemExit('integrity_check failed before backup')\n"
    ...
    run(
        ...
        "order_cols = set()\n"
        "if 'Order' in tables:\n"
        "    order_cols = {r[1] for r in cur.execute('PRAGMA table_info(\"Order\")')}\n"
        "snap = {\n"
        "  'users': count('SELECT count(*) FROM User') if 'User' in tables else None,\n"
        "  'orders': count('SELECT count(*) FROM \"Order\"') if 'Order' in tables else None,\n"
        "  'campaigns': count('SELECT count(*) FROM CouponCampaign') if 'CouponCampaign' in tables else None,\n"
        "  'instances': count('SELECT count(*) FROM CouponInstance') if 'CouponInstance' in tables else None,\n"
        "  'claimed': count(\"SELECT count(*) FROM CouponInstance WHERE status='CLAIMED'\") if 'CouponInstance' in tables else None,\n"
        "  'reserved': count(\"SELECT count(*) FROM CouponInstance WHERE status='RESERVED'\") if 'CouponInstance' in tables else None,\n"
        "  'redeemed': count(\"SELECT count(*) FROM CouponInstance WHERE status='REDEEMED'\") if 'CouponInstance' in tables else None,\n"
        "  'orderIds': [r[0] for r in cur.execute('SELECT id FROM \"Order\"')] if 'Order' in tables else [],\n"
        "  'instanceIds': [r[0] for r in cur.execute('SELECT id FROM CouponInstance')] if 'CouponInstance' in tables else [],\n"
        "  'links': [list(r) for r in cur.execute('SELECT id, couponInstanceId FROM \"Order\" WHERE couponInstanceId IS NOT NULL AND couponInstanceId != \"\"')] if 'couponInstanceId' in order_cols else [],\n"
        "}\n"
        "con.close()\n"
        f"open('{REMOTE_DIR}/.deploy_backup/pre-deploy-snapshot.json','w').write(json.dumps(snap))\n"
        "print('SNAPSHOT', {k: snap[k] for k in ('users','orders','campaigns','instances','claimed','reserved','redeemed')})\n"
        "PY",
        timeout=120,
    )
    ...
    run(
        ...
        "if integrity != 'ok':\n"
        "    raise SystemExit('integrity_check failed after schema push')\n"
        ...
        "n_claimed = cur.execute(\"SELECT count(*) FROM CouponInstance WHERE status='CLAIMED'\").fetchone()[0]\n"
        "n_reserved = cur.execute(\"SELECT count(*) FROM CouponInstance WHERE status='RESERVED'\").fetchone()[0]\n"
        "n_redeemed = cur.execute(\"SELECT count(*) FROM CouponInstance WHERE status='REDEEMED'\").fetchone()[0]\n"
        "print(f'DB_OK users={n_user} orders={n_order} coupons={n_coupon} campaigns={n_camp} instances={n_inst} claimed={n_claimed} reserved={n_reserved} redeemed={n_redeemed}')\n"
        "import json\n"
        f"snap = json.load(open('{REMOTE_DIR}/.deploy_backup/pre-deploy-snapshot.json'))\n"
        "def not_less(name, now):\n"
        "    before = snap.get(name)\n"
        "    if before is None:\n"
        "        return\n"
        "    if now < before:\n"
        "        raise SystemExit(f'{name} decreased {before} -> {now}')\n"
        "not_less('users', n_user)\n"
        "not_less('orders', n_order)\n"
        "not_less('campaigns', n_camp)\n"
        "not_less('instances', n_inst)\n"
        "not_less('claimed', n_claimed)\n"
        "not_less('reserved', n_reserved)\n"
        "not_less('redeemed', n_redeemed)\n"
        "live_order_ids = {r[0] for r in cur.execute('SELECT id FROM \"Order\"')}\n"
        "missing_ids = [i for i in snap.get('orderIds') or [] if i not in live_order_ids]\n"
        "if missing_ids:\n"
        "    raise SystemExit('orders disappeared: ' + ','.join(missing_ids[:10]))\n"
        "live_instance_ids = {r[0] for r in cur.execute('SELECT id FROM CouponInstance')}\n"
        "missing_inst = [i for i in snap.get('instanceIds') or [] if i not in live_instance_ids]\n"
        "if missing_inst:\n"
        "    raise SystemExit('coupon instances disappeared: ' + str(len(missing_inst)))\n"
        "live_links = {tuple(r) for r in cur.execute('SELECT id, couponInstanceId FROM \"Order\" WHERE couponInstanceId IS NOT NULL AND couponInstanceId != \"\"')}\n"
        "missing_links = [link for link in snap.get('links') or [] if tuple(link) not in live_links]\n"
        "if missing_links:\n"
        "    raise SystemExit('order coupon links disappeared: ' + str(len(missing_links)))\n"
        "orphan_orders = cur.execute('SELECT count(*) FROM \"Order\" o LEFT JOIN CouponInstance i ON i.id = o.couponInstanceId WHERE o.couponInstanceId IS NOT NULL AND o.couponInstanceId != \"\" AND i.id IS NULL').fetchone()[0]\n"
        "orphan_redeems = cur.execute('SELECT count(*) FROM CouponInstance i LEFT JOIN \"Order\" o ON o.id = i.redeemedOrderId WHERE i.redeemedOrderId IS NOT NULL AND i.redeemedOrderId != \"\" AND o.id IS NULL').fetchone()[0]\n"
        "if orphan_orders or orphan_redeems:\n"
        "    raise SystemExit(f'coupon relations broken orders={orphan_orders} redeems={orphan_redeems}')\n"
        "import glob, os\n"
